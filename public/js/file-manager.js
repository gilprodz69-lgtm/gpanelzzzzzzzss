...
export function bindFileManager({showModal,modal,formShell,bindForm,toast}){
 ...
 let path='',rows=[],selected=new Set(),bin=false,grid=false,sort='name',ascending=true,busy=false,sequence=0,rootFolders=[],visits=[],visitIndex=-1;
 ...
 function draw(selectionOnly=false){
  ...
  $('#fm-current-path').textContent=bin?'Lixeira':'/public_html'+(path?'/'+path:'');
  ...
  $('#fm-navigation').innerHTML=`<button data-fm="home" class="${!path&&!bin?'active':''}">${icon('files')}public_html</button>${rootFolders.map(r=>`<button data-path="${esc(r.name)}" class="${!bin&&(path===r.name||path.startsWith(r.name+'/'))?'active':''}">${icon('files')}${esc(r.name)}</button>`).join('')}`;
 ...
 async function refresh(){const seq=++sequence;const result=await request(bin?'trash_list':'list');if(seq!==sequence||!el.isConnected)return;rows=result.data;if(!path&&!bin)rootFolders=rows.filter(r=>r.directory);const current={path,bin};if(!visits[visitIndex]||visits[visitIndex].path!==path||visits[visitIndex].bin!==bin){visits=visits.slice(0,visitIndex+1);visits.push(current);visitIndex=visits.length-1;}selected.clear();$('#fm-account').textContent=$('#fm-site').selectedOptions[0].textContent;draw();}
 ...
 async function action(name){
  ...
  if(name==='back'||name==='forward'){const next=visitIndex+(name==='back'?-1:1);if(next<0||next>=visits.length)return;visitIndex=next;({path,bin}=visits[next]);await refresh();return;}
 ...
 $('#fm-site').addEventListener('change',safe(async()=>{path='';bin=false;rootFolders=[];visits=[];visitIndex=-1;rows=[];selected.clear();$('#fm-search').value='';draw();if($('#fm-site').value)await refresh();}));$('#fm-search').addEventListener('input',draw);
